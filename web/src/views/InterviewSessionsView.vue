...
import { deleteInterviewSession, getInterviewSessions } from '../api';
...
async function handleDeleteSession(session) {
  const confirmed = window.confirm('确定删除这场面试复习记录吗？删除后无法恢复。');
  if (!confirmed) return;
            ...
            <tr v-for="session in sessions" :key="session.sessionId">
              ...
              <td>
                <div class="table-actions">
                  ...
                  >
                    {{ deletingSessionId === session.sessionId ? '删除中...' : '删除记录' }}
                  </button>
                </div>
              </td>
            </tr>
